...
const dataPort = 7123; // this is the port to listen on for data from the Photon
...
// Start a TCP Server. This is what receives data from the Particle Argon
...
net.createServer(function (socket) {
	...
	// binary values, so we use setEncoding to read each byte of data as 2 hex digits instead.
	socket.setEncoding('hex');
	
	var outPath = getUniqueOutputPath();
	
	const writer = new wav.FileWriter(outPath, wavOpts);
	...
	socket.on('data', function (data) {
		...
		if (wavOpts.bitDepth == 16) {
			// The Argon sends up unsigned data for both 8 and 16 bit
